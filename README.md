...
- **Все секреты вынесены в `.env`** — никаких хардкодов в `docker-compose.yaml`
- **`.env` в `.gitignore`** — файл с реальными паролями не попадёт в репозиторий
- **Для продакшена** используйте отдельный `docker-compose.prod.yml` и секрет-менеджер (HashiCorp Vault, AWS Secrets Manager, Azure Key Vault и т.д.)
- **Ротируйте секреты** при компрометации или смене команды

### Если секреты попали в историю Git

> Этот репозиторий чистый (нет коммитов), но на будущее:

```bash
# Удаление файла из истории (перезаписывает историю!)
git filter-repo --path .env --invert-paths
# Или через BFG Repo-Cleaner
java -jar bfg.jar --delete-files .env

# После очистки — force push (ТРЕБУЕТ СОГЛАСОВАНИЯ С КОМАНДОЙ!)
git push --force

# ВАЖНО: Срочно смените (ротируйте) все утекшие пароли/ключи на стороне сервисов!
# Очистка git-истории НЕ отменяет компрометации секретов.
```
